...
# The Playpen - Evaluation Pipeline and Training Starter Pack for the LM Playschool Challenge @ EMNLP 2026

The Playpen is an extension of the [clembench](https://github.com/clp-research/clembench) benchmark. It contains everything you need to get started **training** and **evaluating** models in - or from - interaction.  
It will serve as the **official evaluation pipeline** as well as a **training starter pack** for the LM Playschool Challenge, which will be hosted as a shared task at the [LM Playschool Workshop](https://lm-playschool.github.io/) @ EMNLP 2026.
# Index
...
- [Evaluate a model](#evaluate-a-model)
    ...
    - [Finally, the evaluation](#finally-the-evaluation)
- [Train a model](#train-a-model)
    - [Supervised Finetuning](#supervised-finetuning)
        - [Training an Huggingface local model in the Playpen via TRL using Supervised Finetuning](#training-an-huggingface-local-model-in-the-playpen-via-trl-using-supervised-finetuning)
...
If you are interested in running the training examples in the starter pack, you should install TRL.
```bash
pip install '.[trl]'
```

Now that everything is set up, you may follow the next steps to enter deeper into how to train and evaluate a model. You may ignore the training part if you intend to use the Playpen just for your models' evaluation.
# Evaluate a model
## model_registry.json
...
Mind that there is a file of the same name within the cloned _clembench_ folder. You may ignore that one.

The base model you will have to fine-tune for this challenge is already contained in the registry. Here it is:
```json
{
...
Given that in our case we are interested in running Huggingface models, insert your key under `huggingface`:
```json
{
  ...
  "huggingface": {
...
For simplicity, in our training examples we will always rely on the TRL library.
You may find all our examples in the [examples/trl](https://github.com/lm-playpen/playpen/tree/main/examples/trl) folder.
