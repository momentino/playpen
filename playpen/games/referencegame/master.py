...
class ReferenceGameMaster(DialogueGameMaster):
    ...
    def turn(self):
        ...
        self.share_user_message(self.game.instruction_giver, self.game.player_1_prompt_header)
        #self.game.given_instruction.add_user_message(self.game.player_1_prompt_header)

        # log the game master to player 1
        action = {'type': 'send message', 'content': self.game.instruction_giver.get_last_observation()}
        self.log_event(from_="GM", to="Player 1", action=action)

        player_1_prompt, player_1_response, player_1_response_text = self.game.instruction_giver(self.game.instruction_giver.get_observations())
        ...
        self.share_assistant_message(self.game.instruction_giver, player_1_response_text)
        ...
        self.share_user_message(self.game.instruction_follower, self.game.player_2_prompt_header.replace('TARGET_EXPRESSION', player_1_response_text))
        ...
        action = {'type': 'send message', 'content': self.game.instruction_follower.get_last_observation()}
        self.log_event(from_="GM", to="Player 2", action=action)

        player_2_prompt, player_2_response, player_2_response_text = self.game.instruction_follower(self.game.instruction_follower.get_observations())

        self.share_assistant_message(self.game.instruction_follower, player_2_response_text)
