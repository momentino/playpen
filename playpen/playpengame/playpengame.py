...
import playpen.playpengame.metrics as ms
...
class Player(abc.ABC):
    ...
    def observe(self, observation, reward, termination, truncation, info):
        self.agent.observe(observation, reward, termination, truncation, info)
    ...
    def get_last_observation(self):
        return self.agent.get_last_observation()
...
class GameResourceLocator(abc.ABC):
    ...
    def __init__(self, name: str):
        ...
        """
        self.name = name
        self.logger = playpengame.get_logger(self.__class__.__module__)
...
class DialogueGameMaster(GameMaster):
    ...
    def prompt(self, player: Player, is_reprompt=False):
        ...
        _prompt, _response, response_message = player.act()
    ...
    def share_user_message(self, player: Player, utterance: str):
        ...
        player.observe(observation, None, None, None, None)
    ...
    def run(self, player_agents: List[Agent], results_dir: str = None):
        ...
        for experiment_idx, experiment in enumerate(experiments):
            ...
            if not dialogue_partners:
                message = (f"{self.name}: 'dialogue_partners' not 'models' given as run arg")
                stdout_logger.error(message)
                raise ValueError(message)

            dialogue_lineup_desc = ""
            for agent in dialogue_partners:
                dialogue_lineup_desc += agent.get_name()+"--"
            dialogue_lineup_desc = dialogue_lineup_desc[:-2]

            episode_counter = 0

            self.logger.info("Activity: %s Experiment: %s Partners: %s Episode: %d",
                             self.name, experiment_name, dialogue_lineup_desc, episode_counter)
            ...
            experiment_config["dialogue_partners"] = dialogue_lineup_desc
            ...
            for game_instance in tqdm(game_instances, desc="Playing games"):
                ...
                self.store_results_file(game_instance,
                                        ...
                                        root_dir=results_root)
                try:
                    game_master = self.create_game_master(experiment_config, dialogue_partners)
                    game_master.setup(**game_instance)
                    game_master.play()
                    game_master.store_records(results_root, dialogue_lineup_desc, episode_dir)
                except Exception:  # continue with other episodes if something goes wrong
...
def num_players(game_name: str) -> Tuple[int, int]:
    gm = find_benchmark(game_name)
    min_players = gm.get_min_num_players()
    max_players = gm.get_max_num_players()
